...
##Running the example on Pivotal HD cluster

###pre-requisite
* Must have executed the sample in eclipse.

###Customize configuration
Change `hadoop.properties` to reflect the Pivotal HDFS Namenode instead of localhost as shown below: 

```xml
hd.fs=hdfs://localhost:9000
```

Make sure the PROJECT_DIR is replaced with current project root directory 
...
```bash
tar -zcvf spring-hadoop-sample.tar.gz target/* input/*
scp sampring-hadoop-sample.tar.gz gpadmin@DATA_NODE:/home/gpadmin/spring-hadoop-sample.tar.gz 
```
...
```bash
cd /home/gpadmin
mkdir spring-hadoop-sample
cd spring-hadoop-sample
tar -zxvf ../spring-hadoop-sample.tar.gz 
```
