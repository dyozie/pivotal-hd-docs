...
--------------------------------------------------------------
The given dataset has information about the Businesses. Each business is located in a city. This use case is a simple example on how to get count how many businesses are there in a city. This is not a big data problem, however illsutrated as a basic mapreduce application.
...
The data is obtained from [here](https://www.yelp.com/academic_dataset, "Yelp Academic Data Set").

Detailed information about the various objects and their fields can be referred from the above link.

Approach
--------
A single data record about the business is shown below:
The Data is in JSON format. 

Following are the steps to the exercise:

*  Understand the InputFormat and desiging the custom input format class, if required.
*  Design the Mapper
*  Design the Reducer
...
###Step 2: Understand the InputFormat
A sample record is shown below:

```xml
{"business_id": "G2Re2E5Jkv_UF1xrenSsDg", "full_address": "4500 E Cactus Rd\nPhoenix, AZ 85032", "open": true, "categories": ["Department Stores", "Fashion", "Shopping"], "city": "Phoenix", "review_count": 11, "name": "Macy's", "neighborhoods": [], "longitude": -111.9820907, "state": "AZ", "stars": 4.0, "latitude": 33.599323599999998, "type": "business"}
```

Since the data is JSON and we need to parse the JSON line to get the values for various different fields. A custom InputFormat is written by extending the FileInputFormat and overiding the createRecordReader method. 

The YelpDataInputFormat is shown below:
```java
...
#### Mapper code:
Copy the following code into map method of CityBusinessMapper class.
...
copy the following code into the Reduce methof of the class CityBusinessReducer.java
...
Copy the following code to the main method of CityBusinessDriver
